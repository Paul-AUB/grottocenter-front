...
const MapInternals = ({ geoJson, massifId }) => {
  ...
  const fetchMarkers = useCallback(() => {
    ...
    fetch(makeUrl(getMapEntrancesUrl, criteria), { signal })
      ...
      .catch(err => {
        if (err.name !== 'AbortError') console.error('Failed to fetch map markers:', err); // eslint-disable-line no-console
      });
  }, [map, massifId]);
  ...
  useEffect(() => {
    ...
    fetch(makeUrl(getMapEntrancesCoordinatesUrl, { sw_lat: sw.lat, sw_lng: sw.lng, ne_lat: ne.lat, ne_lng: ne.lng, massif: massifId }), { signal: controller.signal })
      .then(r => (r.ok ? r.json() : []))
      .then(data => {
