...
import { caverIcon, entranceIcon, networkIcon, bibliographyIcon } from '../../../assets/icons';
        ...
        {(hasAddress || organization.mail || organization.url) && (
          <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, bgcolor: 'grey.50' }}>
            <InfoSection title={formatMessage({ id: 'Contact' })}>
              {hasAddress && (
                <InfoRow>
                  <LocationIcon color="primary" sx={{ fontSize: 24, flexShrink: 0 }} />
                  <Typography>{addressText}</Typography>
                </InfoRow>
              )}
              {organization.mail && (
                <InfoRow>
                  <EmailIcon color="primary" sx={{ fontSize: 24, flexShrink: 0 }} />
                  <Typography>
